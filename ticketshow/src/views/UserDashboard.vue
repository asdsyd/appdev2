...
if((startTime.value!==null&&endTime.value===null)||(startTime.value===null&&endTime.value!==null)){
return 
}
...
obj['end'] = endTime.value
}
is_loading.value = true
axios.post("/user/getVenues",obj)
      .then((res) => {
...
onBeforeMount(() => {
  if (store.state.venues.length > 0) {
    ...
    axios
      .post("/user/getVenues",{"msg":"request"})
      .then((res) => {
        const { venues } = res.data;
        console.log(venues)
        store.commit("addvenues", venues);
